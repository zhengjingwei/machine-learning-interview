...
- [ ] [7-1-3  牛顿法和梯度法有什么区别？](#7-1-1)
  ...
  梯度法对目标函数进行一阶泰勒展开，梯度就是目标函数的一阶信息；

  牛顿法对目标函数进行二阶泰勒展开，Hessian矩阵就是目标函数的二姐信息。

  牛顿法的收敛速度一般要远快于梯度法，但是在高维情况下Hessian矩阵求逆的计算复杂度很大，而且当目标函数非凸时，牛顿法有可能会收敛到鞍点。因为梯度法旨在朝下坡移动，而牛顿法目标是寻找梯度为0的点。
